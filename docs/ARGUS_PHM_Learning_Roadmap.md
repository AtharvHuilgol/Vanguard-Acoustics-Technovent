...
## Student Team Curriculum — Atharv Huilgol · Pratiksha Patil · Arya Mandlik
...
Team responsibilities (3 members, cross-disciplinary):
  Atharv Huilgol  (EnTC — Embedded ML):   Modules 1 & 3 — FreeRTOS firmware + TinyML pipeline
  Pratiksha Patil (EnTC — DSP/Hardware):  Modules 1 & 2 — SPI hardware layer + FFT/spectrogram
  Arya Mandlik    (Mechanical):            Module 4 — gear kinematics, rig build, fault injection
  All three:                               Integration week — full stack run end-to-end
```
...
Our team composition explicitly fulfils the competition's cross-disciplinary and diversity scoring rubrics. Two Electronics & Telecommunications (EnTC) engineers cover the firmware and signal-processing stack; one Mechanical Engineering specialist anchors hardware reliability, gear kinematics, and the physical Demo Day test bench. Every layer of ARGUS-PHM has a dedicated specialist owner.

| Team Member | Background | Competition Role | Key Technical Focus |
|---|---|---|---|
| Atharv Huilgol | Electronics & Telecommunications (EnTC) | Lead Embedded ML Engineer | TinyML quantisation, TensorFlow Lite deployment, RTOS task scheduling, bare-metal footprint optimisation |
| Pratiksha Patil | Electronics & Telecommunications (EnTC) | DSP & Hardware Architect | High-frequency SPI sensor interfacing, Fast Fourier Transform (FFT) processing, spectrogram generation |
| Arya Mandlik | Mechanical Engineering | Aerospace Reliability & Test Rig Engineer | Planetary gear kinematics, acoustic emission modelling, fault injection, physical Demo Day test bench construction |

**Why the mechanical background is load-bearing, not supplementary:** Correct Gear Mesh Frequency (GMF) calculation for the specific gear train on the test bench, epoxy sensor bonding procedure selection (surface finish, cure protocol), and fault severity calibration (notch depth to crack-propagation relationship) all require mechanical engineering domain knowledge. The DSP and TinyML layers built by Atharv and Pratiksha are only as physically valid as the mechanical ground truth that Arya provides. This is explicit jury differentiation — most competing teams will be purely software-oriented.

---

## Module 1: Embedded Systems, High-Speed SPI & FreeRTOS Core

**Owners: Atharv Huilgol (FreeRTOS lead) · Pratiksha Patil (SPI hardware lead) — Estimated duration: 4–5 weeks**
...
**Owner: Pratiksha Patil — Estimated duration: 3–4 weeks**
...
**Owner: Atharv Huilgol — Estimated duration: 5–6 weeks**
...
**Owner: Arya Mandlik — Estimated duration: 6–8 weeks (overlaps with Module 3)**
...
*Team: Atharv Huilgol (EnTC) · Pratiksha Patil (EnTC) · Arya Mandlik (Mechanical)*
